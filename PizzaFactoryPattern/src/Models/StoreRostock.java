...
public class StoreRostock extends Store{
    ...
    public Pizza cook(PizzaVariation p) {
        if(p == PizzaVariation.Salami) return new PizzaSalami(PizzaLocalVariation.RostockSalami);
        if(p == PizzaVariation.Calzone) return new PizzaCalzone(PizzaLocalVariation.RostockCalzone);
